...
public class WalkingMapActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_walkingmap);

		// 获取传递进来的值 表示采用第几套方案 默认为0
		Intent intent = getIntent();
		getIntent = intent.getIntExtra("index", 0);
//w		title = "我的位置-"+intent.getStringExtra("END_POINT");
		System.out.println("###传递进来的值为" + getIntent);
	...
	private void init() {
		UiActivity.setMyTitle(R.layout.title_normal, this);// 设置标题
		...
		mBaiduMap.setOnMapLoadedCallback(new OnMapLoadedCallback() {
			...
			public void onMapLoaded() {
				WalkingRouteResult result = myTransitRouteResult
						.getMyWalkingRouteResult();
				WalkingRouteLine walingRouteLine = result.getRouteLines().get(
						getIntent);// 获取该步行方案
				WalkingRouteOverlay overlay = new MyWalkingRouteOverlay(mBaiduMap);
				// 创建步行路线规划线路覆盖物
				overlay.setData(walingRouteLine);
				// 将步行路线规划覆盖物添加到地图中
				overlay.addToMap();
				overlay.zoomToSpan();
